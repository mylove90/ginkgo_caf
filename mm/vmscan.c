					   ...
					   enum zone_type highest_zoneidx)
{
	if (pgdat->kswapd_highest_zoneidx == MAX_NR_ZONES)
		return highest_zoneidx;

	return max(pgdat->kswapd_highest_zoneidx, highest_zoneidx);
}
...
kswapd_try_sleep:
		...
		alloc_order = reclaim_order = pgdat->kswapd_order;
		highest_zoneidx = kswapd_highest_zoneidx(pgdat, 0);
		pgdat->kswapd_order = 0;
		pgdat->kswapd_highest_zoneidx = MAX_NR_ZONES;
...
void wakeup_kswapd(struct zone *zone, int order, enum zone_type highest_zoneidx)
{
	...
	if (!cpuset_zone_allowed(zone, GFP_KERNEL | __GFP_HARDWALL))
		return;
	pgdat = zone->zone_pgdat;
	pgdat->kswapd_highest_zoneidx = kswapd_highest_zoneidx(pgdat,
							   highest_zoneidx);
